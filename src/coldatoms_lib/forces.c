...
void coulomb_force_(const double *positions, double charge, int num_ptcls,
	double delta, double k, double *forces)
{
	k *= charge * charge;
...
static void transpose(
	const double *restrict x, int m, int n,
	double *restrict y)
{
	for (int i = 0; i < m; ++i) {
		for (int j = 0; j < n; ++j) {
...
static void coulomb_force_chunked(const double * restrict positions,
	double charge, int num_ptcls, double delta, double k,
	double * restrict forces)
{
	...
	for (int i = 0; i < num_chunks; ++i) {
		...
		transpose(
			positions + i * NUM_COMPONENTS * CHUNK_SIZE,
			CHUNK_SIZE, NUM_COMPONENTS,
			&x0[0][0]);
		double f[NUM_COMPONENTS][CHUNK_SIZE] = {{0.0}};
		for (int j = 0; j < num_chunks; ++j) {
			double x1[NUM_COMPONENTS][CHUNK_SIZE];
			transpose(
				positions + j * NUM_COMPONENTS * CHUNK_SIZE,
				CHUNK_SIZE, NUM_COMPONENTS,
				&x1[0][0]);

	...
	double *restrict x, double * restrict v)
{
	...
	double *x0 = &x[0];
	double *x1 = &x[num_ptcls];
	double *v0 = &v[0];
	double *v1 = &v[num_ptcls];

	for (int i = 0; i < num_ptcls; ++i) {
		x0[i] = x0[i] + (sinTheta * v0[i] + (cosTheta - 1.0) * v1[i]) / omegaB;
		x1[i] = x1[i] + (-(cosTheta - 1.0) * v0[i] + sinTheta * v0[i]) / omegaB;
	...
	double *restrict x, double * restrict v)
{
	...
	for (int i = 0; i < num_ptcls; ++i) {
		...
		x0[i] = x0[i] + (sinTheta * v0[i] + (cosTheta - 1.0) * v1[i]) / omegaB[i];
		x1[i] = x1[i] + (-(cosTheta - 1.0) * v0[i] + sinTheta * v0[i]) / omegaB[i];
		double vx_tmp = cosTheta * v0[i] - sinTheta * v1[i];
		double vy_tmp = sinTheta * v0[i] + cosTheta * v1[i];
		v0[i] = vx_tmp;
		v1[i] = vy_tmp;
